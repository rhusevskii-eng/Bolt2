...
const palette = [
  ...
  'from-cyan-600 to-cyan-800', 'from-sky-600 to-sky-800',
...
export default function BuyersTicker() {
  return (
    <div className="relative py-3 border-y border-blue-800/50 overflow-hidden" style={{ background: 'rgba(4,14,28,0.85)' }}>
      <div className="absolute left-0 top-0 h-full w-28 pointer-events-none z-10" style={{ background: 'linear-gradient(to right, #05121f, transparent)' }} />
      <div className="absolute right-0 top-0 h-full w-28 pointer-events-none z-10" style={{ background: 'linear-gradient(to left, #05121f, transparent)' }} />
      ...
      <div className="flex animate-ticker" style={{ width: 'max-content' }}>
        {items.map((name, i) => (
          <div key={i} className="flex items-center gap-2.5 mx-5 flex-shrink-0">
            ...
            <span className="text-sm font-medium text-blue-200/80 whitespace-nowrap">{name}</span>
            <span className="w-0.5 h-0.5 rounded-full bg-blue-700 flex-shrink-0 ml-1" />
          </div>
        ))}
      </div>
    </div>
